/**
 * Flat translation dictionary for the entire app.
 *
 * Conventions:
 *  - Keys are dot-namespaced by feature: `settings.sync`, `library.title`.
 *  - Values are plain strings; `{var}` placeholders are interpolated at
 *    call sites via `t(key, { var: ... })`.
 *  - Always add the key to all three languages. Missing keys fall through
 *    to the `ko` default (see LanguageProvider.resolve), never throw.
 *
 * Keep this file boring and exhaustive — it's the source of truth for every
 * user-facing string. Grep the app for hardcoded Korean whenever a feature
 * lands to catch drift.
 */

import type { Language } from "./config";

type Dict = Record<string, string>;

const ko: Dict = {
  // --- navigation
  "nav.home": "홈",
  "nav.library": "서재",
  "nav.capture": "기록",
  "nav.settings": "설정",

  // --- common
  "common.save": "저장",
  "common.cancel": "취소",
  "common.delete": "삭제",
  "common.edit": "수정",
  "common.share": "공유",
  "common.retry": "다시 시도",
  "common.close": "닫기",
  "common.confirm": "확인",
  "common.back": "뒤로",
  "common.loading": "불러오는 중...",
  "common.ok": "확인",

  // --- time (relative)
  "time.never": "아직 없음",
  "time.just_now": "방금 전",
  "time.seconds_ago": "{n}초 전",
  "time.minutes_ago": "{n}분 전",
  "time.hours_ago": "{n}시간 전",
  "time.days_ago": "{n}일 전",

  // --- home
  "home.brand": "Quote",
  "home.daily_title": "오늘의 문장",
  "home.shuffle": "다른 문장 보기",
  "home.recent_title": "최근 기록",
  "home.empty": "아직 기록이 없어요",
  "home.empty_cta": "첫 번째 문장을 기록해보세요",
  "home.greeting_morning": "좋은 아침이에요",
  "home.greeting_afternoon": "좋은 오후예요",
  "home.greeting_evening": "좋은 저녁이에요",
  "home.welcome_content": "첫 문장을 기록해보세요. 카메라로 찍으면 자동으로 글자가 인식돼요.",
  "home.welcome_book": "환영합니다",

  // --- library
  "library.title": "서재",
  "library.count": "{books}권 · 문장 {quotes}개",
  "library.empty": "아직 책이 없어요",
  "library.empty_initial": "첫 문장을 기록하면 책이 여기에 꽂혀요",
  "library.empty_search": "검색 결과가 없어요",
  "library.search_placeholder": "책 제목, 저자 검색...",
  "library.quote_count": "문장 {count}개",
  "library.unassigned": "미분류 문장",
  "library.unassigned_short": "미분류",
  "library.unassigned_desc": "책 정보 없이 기록한 문장",
  "library.unassigned_count": "{count}개의 문장",
  "library.last_quote_at": "최근 {time}",

  // --- capture
  "capture.title": "기록하기",
  "capture.subtitle": "한 줄의 영감을 남겨보세요",
  "capture.from_camera": "사진",
  "capture.from_photo": "사진",
  "capture.from_voice": "음성",
  "capture.type_directly": "직접 입력",
  "capture.quote_placeholder": "인상 깊은 문장을 입력하세요.",
  "capture.book_info_label": "책 정보",
  "capture.book_title_placeholder": "책 제목",
  "capture.book_author_placeholder": "저자",
  "capture.recent_books": "최근 읽은 책",
  "capture.thought_label": "나의 생각",
  "capture.thought_placeholder": "이 문장에 대한 생각을 자유롭게 적어보세요.",
  "capture.save": "저장하기",
  "capture.save_error": "저장에 실패했어요. 잠시 후 다시 시도해 주세요.",
  "capture.saving": "저장 중...",
  "capture.ocr_title": "사진으로 기록",
  "capture.ocr_from_camera": "카메라로 촬영",
  "capture.ocr_from_library": "사진에서 선택",
  "capture.ocr_recognizing": "문장 인식 중…",
  "capture.ocr_failed": "인식 실패",
  "capture.ocr_retake": "다시",
  "capture.ocr_retry": "다시 시도",
  "capture.ocr_page_label": "{n}번째 사진",
  "capture.ocr_remove_page": "삭제",
  "capture.ocr_add_camera_page": "한 장 더 촬영",
  "capture.ocr_add_library_page": "사진 추가",
  "capture.ocr_selector_hint": "기록할 문장을 탭하세요 (여러 개 선택 가능)",
  "capture.ocr_no_sentences": "인식된 문장이 없어요. 다시 촬영해 주세요.",
  "capture.ocr_use_selected": "선택한 문장 사용하기",
  "capture.saved": "저장되었어요",
  "capture.missing_content": "내용을 입력해 주세요",

  // --- signin
  "signin.title": "로그인",
  "signin.subtitle_email": "이메일로 인증 코드 또는 매직 링크를 보내드릴게요",
  "signin.subtitle_code": "메일로 받은 인증 코드를 입력하거나, 메일 속 링크를 눌러 주세요.",
  "signin.already_in": "이미 로그인되어 있어요:",
  "signin.email_placeholder": "you@example.com",
  "signin.send_code": "코드 / 링크 보내기",
  "signin.sending": "보내는 중...",
  "signin.code_sent": "메일함을 확인해 주세요. 링크를 눌러도 되고, 코드를 입력해도 됩니다.",
  "signin.code_hint": "메일로 받은 인증 코드를 입력해 주세요.",
  "signin.code_placeholder": "인증 코드",
  "signin.verify": "코드로 로그인",
  "signin.verifying": "확인하는 중...",
  "signin.change_email": "이메일 바꾸기",
  "signin.resend": "코드 다시 받기",
  "signin.error_rate_limit": "메일을 너무 자주 요청했어요. 잠시 후(약 1시간) 다시 시도해 주세요.",
  "signin.error_expired": "코드가 만료되었어요. 메일을 다시 요청해 주세요.",
  "signin.error_empty_code": "인증 코드를 입력해 주세요.",
  "signin.error_invalid_code": "코드가 올바르지 않아요. 다시 확인해 주세요.",
  "signin.error_generic": "잠시 후 다시 시도해 주세요.",

  // --- settings (account)
  "settings.title": "설정",
  "settings.account": "계정",
  "settings.not_signed_in": "로그인하지 않음 (로컬 저장만)",
  "settings.sign_in": "로그인",
  "settings.sign_out": "로그아웃",
  "settings.delete_account": "계정 삭제",
  "settings.delete_account_confirm_title": "계정을 삭제할까요?",
  "settings.delete_account_confirm_body":
    "클라우드에 동기화된 기록·이미지와 이 기기의 앱 데이터가 삭제됩니다. 되돌릴 수 없어요.",
  "settings.delete_account_confirm": "삭제하기",
  "settings.delete_account_cancel": "취소",
  "settings.delete_account_busy": "삭제 중...",
  "settings.delete_account_error": "계정 삭제에 실패했어요. 잠시 후 다시 시도해 주세요.",
  "settings.legal": "정보",
  "settings.privacy": "개인정보 처리방침",
  "settings.support": "고객 지원",

  // --- settings (Quote Pro)
  "settings.pro": "Quote Pro",
  "settings.pro_desc": "워터마크 없는 공유 · 노트를 Markdown / Obsidian / Notion으로 아카이브",
  "settings.pro_desc_active": "Pro가 켜져 있어요. 아카이브와 워터마크 없는 공유를 사용할 수 있어요.",
  "settings.pro_row_watermark": "이미지 공유 시 워터마크 제거",
  "settings.pro_row_archive": "Markdown · Obsidian · Notion 내보내기",
  "settings.pro_cta": "Pro 알아보기",
  "settings.pro_archive_cta": "아카이브 내보내기",
  "settings.pro_manage": "구독 관리 · 복원",
  "settings.pro_soon": "Pro 준비 중이에요",

  // --- settings (support the developer)
  "settings.support_dev": "개발자 응원하기",
  "settings.support_dev_desc": "Quote는 혼자 만드는 앱이에요. 작은 응원이 큰 힘이 됩니다.",
  "settings.support_dev_not_pro": "후원은 기능 해금과 무관한 일회성 응원이에요.",
  "settings.support_dev_cta": "후원하기",
  "settings.support_dev_soon": "후원 준비 중이에요",

  // --- Quote Pro sheet
  "pro.title": "Quote Pro",
  "pro.subtitle": "기록은 그대로 두고, 공유와 아카이브만 한 단계 더.",
  "pro.badge_active": "이용 중",
  "pro.feature_watermark": "이미지 공유 시 워터마크 제거",
  "pro.feature_markdown": "독서노트 Markdown(.md) 내보내기",
  "pro.feature_obsidian": "Obsidian으로 바로 새 노트 열기",
  "pro.feature_notion": "Notion 페이지로 아카이브",
  "pro.subscribe_cta": "월 {price}으로 시작",
  "pro.restore": "구매 복원",
  "pro.restore_ok": "Pro가 복원되었어요.",
  "pro.restore_none": "복원할 구독을 찾지 못했어요.",
  "pro.purchase_error": "구독을 완료하지 못했어요. 스토어 상품이 준비됐는지 확인하거나 잠시 후 다시 시도해 주세요.",
  "pro.preview_note": "미리보기 모드 — 실제 결제 없이 Pro UI를 확인할 수 있어요.",

  // --- archive export (Pro)
  "archive.title": "아카이브 내보내기",
  "archive.subtitle": "Quote에 저장된 독서노트를 외부 앱으로 보냅니다. Pro 전용.",
  "archive.md": "Markdown 파일",
  "archive.md_desc": ".md 파일을 공유하거나 저장해요",
  "archive.md_ok": "Markdown 파일을 준비했어요.",
  "archive.obsidian": "Obsidian",
  "archive.obsidian_desc": "Obsidian에서 새 노트로 열기 (길면 파일 공유)",
  "archive.obsidian_uri_ok": "Obsidian을 열었어요. 앱이 없다면 Markdown 파일로 다시 시도해 주세요.",
  "archive.obsidian_file_ok": "노트가 길어서 .md 파일로 공유해요. vault 폴더에 저장해 주세요.",
  "archive.copy": "클립보드에 복사",
  "archive.copy_desc": "Markdown을 복사해 Notion 등에 붙여넣기",
  "archive.copy_ok": "클립보드에 복사했어요.",
  "archive.notion": "Notion으로 보내기",
  "archive.notion_desc": "연동한 페이지 아래에 아카이브 페이지 생성",
  "archive.notion_show_config": "Notion 연동 설정",
  "archive.notion_hide_config": "연동 설정 접기",
  "archive.notion_help": "Notion Internal Integration 토큰과, Integration을 초대한 부모 페이지 ID가 필요해요. 토큰은 이 기기에만 저장됩니다.",
  "archive.notion_token_ph": "secret_… 또는 ntn_… 토큰",
  "archive.notion_page_ph": "부모 페이지 ID (32자)",
  "archive.notion_save": "연동 저장",
  "archive.notion_need_config": "먼저 Notion 토큰과 부모 페이지 ID를 저장해 주세요.",
  "archive.notion_ok": "Notion에 페이지를 만들었어요.",
  "archive.notion_ok_url": "Notion에 만들었어요: {url}",
  "archive.notion_error": "Notion 오류: {msg}",
  "archive.empty": "내보낼 노트가 아직 없어요.",
  "archive.cancelled": "공유를 취소했어요.",
  "archive.error": "내보내기에 실패했어요. 잠시 후 다시 시도해 주세요.",

  // --- tip sheet (voluntary IAP)
  "tip.title": "개발자에게 커피 한 잔",
  "tip.subtitle": "앱이 도움이 되었다면 자유롭게 응원해 주세요.",
  "tip.tier_small": "작은 응원",
  "tip.tier_medium": "든든한 응원",
  "tip.tier_large": "큰 응원",
  "tip.unavailable": "후원 설정을 불러올 수 없어요. 앱을 최신 버전으로 업데이트한 뒤 다시 시도해 주세요.",
  "tip.error": "상품 정보를 불러오지 못했어요.",
  "tip.retry": "다시 시도",
  "tip.purchase_error": "결제를 완료하지 못했어요. 잠시 후 다시 시도해 주세요.",
  "tip.thanks_title": "정말 고맙습니다",
  "tip.thanks_body": "보내주신 응원은 Quote를 계속 다듬는 데 큰 힘이 됩니다.",

  // --- settings (storage)
  "settings.store_images": "원본 이미지 보관",
  "settings.store_images_desc": "꺼두면 인식 후 원본은 저장하지 않아요",

  // --- settings (language)
  "settings.language": "언어",
  "settings.language_desc": "앱 표시 언어를 바꿉니다",

  // --- settings (sync panel)
  "settings.sync": "동기화",
  "settings.sync_now": "지금 동기화",
  "settings.sync_network": "네트워크",
  "settings.sync_online": "온라인",
  "settings.sync_offline": "오프라인",
  "settings.sync_login": "로그인",
  "settings.sync_login_none": "없음 (로컬만)",
  "settings.sync_last": "마지막 동기화",
  "settings.sync_pending": "대기 중 업로드",
  "settings.sync_pending_count": "{count}건",
  "settings.sync_failed": "동기화 실패",
  "settings.sync_retry_failed": "다시 시도",
  "settings.sync_recent": "최근 결과",

  // --- settings (cover retry)
  "settings.covers": "표지 자동 찾기",
  "settings.covers_retry": "표지 다시 찾기",
  "settings.covers_searching": "찾는 중...",
  "settings.covers_desc":
    "표지가 없는 책은 제목 언어에 따라 카카오 → 네이버 → Open Library → 구글 순으로 찾습니다. 카카오·네이버는 키가 있을 때만 먼저 쓰이고, 없으면 Open Library·구글을 사용합니다. 키는 `.env.local`에 넣은 뒤 앱을 다시 빌드해야 반영됩니다. 인터넷이 필요합니다.",
  "settings.covers_on": "사용",
  "settings.covers_off": "미사용",
  "settings.covers_providers_status":
    "도서 API: 카카오 {kakao}, 네이버 {naver}, Open Library {openLibrary}, 구글 {google}",
  "settings.covers_total": "총 책",
  "settings.covers_already": "이미 표지 있음",
  "settings.covers_tried": "시도",
  "settings.covers_result": "성공 / 실패",
  "settings.covers_book_count": "{count}권",
  "settings.covers_no_books": "서재에 아직 책이 없어요. 먼저 문장을 저장해 주세요.",
  "settings.covers_match": "표지 매칭",
  "settings.covers_fail": "매칭 실패",
  "settings.covers_and_more": "...외 {count}건",

  // --- privacy (in-app)
  "privacy.title": "개인정보 처리방침",
  "privacy.updated": "최종 업데이트: 2026-07-23",
  "privacy.intro":
    "Quote는 책의 문장을 기록·보관하는 앱입니다. Quote는 Tealdot(tealdot.dev)이 운영합니다.",
  "privacy.s1_title": "1. 수집하는 정보",
  "privacy.s1_email": "이메일 — 로그인(매직 링크 / 인증 코드) 시",
  "privacy.s1_content": "문장, 메모, 책 제목·저자 등 사용자가 저장한 콘텐츠",
  "privacy.s1_photos":
    "사진 — OCR용. ‘원본 이미지 보관’을 켠 경우에만 클라우드에 업로드될 수 있음",
  "privacy.s1_local": "기기 로컬 저장(오프라인용)",
  "privacy.s2_title": "2. 이용 목적",
  "privacy.s2_auth": "계정 인증 및 기기 간 동기화",
  "privacy.s2_quotes": "문장 기록·서재 관리",
  "privacy.s2_covers": "책 표지 자동 매칭(외부 도서 API)",
  "privacy.s3_title": "3. 제3자",
  "privacy.s3_supabase": "Supabase — 인증, DB, (선택) 이미지 저장",
  "privacy.s3_books": "Google Books 및(설정 시) Kakao/Naver 도서 API",
  "privacy.s3_no_ads": "광고·추적 SDK는 쓰지 않으며, 개인정보를 판매하지 않습니다.",
  "privacy.s4_title": "4. 보관 및 삭제",
  "privacy.s4_body":
    "설정에서 계정 삭제를 요청하면 계정·동기화 데이터·관련 클라우드 이미지와 이 기기 앱 데이터가 삭제됩니다.",
  "privacy.s5_title": "5. 문의",
  "privacy.s5_contact": "웹사이트에서 문의하기",

  // --- book detail
  "book.unknown_title": "제목 없음",
  "book.back_label": "돌아가기",
  "book.cover_alt": "{title} 표지",
  "book.back_to_library": "서재",
  "book.quote_count": "문장 {count}개",
  "book.empty_quotes": "이 책의 문장이 아직 없어요",
  "book.add_quote": "이 책에 문장 추가",
  "book.add_quote_hint": "이 책에 담을 문장을 입력하세요.",

  // --- quote actions
  "quote.action_title": "이 문장에 대한 작업",
  "quote.edit": "수정",
  "quote.delete": "삭제",
  "quote.deleting": "삭제 중...",
  "quote.delete_confirm_title": "이 문장을 삭제할까요? 되돌릴 수 없어요.",
  "quote.cancel": "취소",
  "quote.action_default_title": "동작 선택",
  "quote.deleted": "삭제되었어요",
  "quote.expand": "펼쳐보기",
  "quote.collapse": "접기",
  "quote.favorite": "즐겨찾기",
  "quote.share_image": "이미지로 공유",

  // --- edit sheet
  "edit.title": "문장 수정",
  "edit.save": "저장",
  "edit.saving": "저장 중...",
  "edit.cancel": "취소",
  "edit.book_title_placeholder": "책 제목 (비우면 연결 해제)",

  // --- share (Phase C placeholders, used later)
  "share.title": "이미지로 공유",
  "share.size_square": "정사각 (1080×1350)",
  "share.size_story": "스토리 (1080×1920)",
  "share.save_photo": "사진에 저장",
  "share.share": "공유",
  "share.generating": "이미지 생성 중...",
  "share.promoted_notice":
    "문장이 길어 세로형(9:16) 사이즈로 자동 전환했어요.",
  "share.too_long_warning":
    "문장이 매우 길어 일부가 잘릴 수 있어요. 긴 문장 분할 저장은 준비 중이에요.",
  "share.select_sentences_hint": "공유할 문장을 골라보세요",
  "share.sentences_none_selected": "선택된 문장이 없어요",
  "share.watermark_off_pro": "워터마크 없이 공유",
  "share.watermark_upsell": "워터마크 없이 공유하려면 Quote Pro",
};

const en: Dict = {
  "nav.home": "Home",
  "nav.library": "Library",
  "nav.capture": "Capture",
  "nav.settings": "Settings",

  "common.save": "Save",
  "common.cancel": "Cancel",
  "common.delete": "Delete",
  "common.edit": "Edit",
  "common.share": "Share",
  "common.retry": "Retry",
  "common.close": "Close",
  "common.confirm": "OK",
  "common.back": "Back",
  "common.loading": "Loading...",
  "common.ok": "OK",

  "time.never": "never",
  "time.just_now": "just now",
  "time.seconds_ago": "{n}s ago",
  "time.minutes_ago": "{n}m ago",
  "time.hours_ago": "{n}h ago",
  "time.days_ago": "{n}d ago",

  "home.brand": "Quote",
  "home.daily_title": "Quote of the Day",
  "home.shuffle": "Shuffle",
  "home.recent_title": "Recent",
  "home.empty": "Nothing captured yet",
  "home.empty_cta": "Capture your first quote",
  "home.greeting_morning": "Good morning",
  "home.greeting_afternoon": "Good afternoon",
  "home.greeting_evening": "Good evening",
  "home.welcome_content": "Capture your first line. Snap a photo and the text is recognized automatically.",
  "home.welcome_book": "Welcome",

  "library.title": "Library",
  "library.count": "{books} books · {quotes} quotes",
  "library.empty": "No books yet",
  "library.empty_initial": "Capture your first quote and books appear here",
  "library.empty_search": "No matches",
  "library.search_placeholder": "Search title or author...",
  "library.quote_count": "{count} quotes",
  "library.unassigned": "Unassigned quotes",
  "library.unassigned_short": "Unassigned",
  "library.unassigned_desc": "Quotes saved without a book",
  "library.unassigned_count": "{count} quotes",
  "library.last_quote_at": "Last {time}",

  "capture.title": "Capture",
  "capture.subtitle": "Save a line of inspiration",
  "capture.from_camera": "Photo",
  "capture.from_photo": "Photo",
  "capture.from_voice": "Voice",
  "capture.type_directly": "Type",
  "capture.quote_placeholder": "Enter the passage that stayed with you.",
  "capture.book_info_label": "Book",
  "capture.book_title_placeholder": "Book title",
  "capture.book_author_placeholder": "Author",
  "capture.recent_books": "Recent books",
  "capture.thought_label": "Your thought",
  "capture.thought_placeholder": "Write down whatever you'd like to remember about it.",
  "capture.save": "Save",
  "capture.save_error": "Couldn’t save. Please try again shortly.",
  "capture.saving": "Saving...",
  "capture.ocr_title": "Capture from photo",
  "capture.ocr_from_camera": "Use camera",
  "capture.ocr_from_library": "Choose photo",
  "capture.ocr_recognizing": "Recognizing text…",
  "capture.ocr_failed": "Couldn't recognize",
  "capture.ocr_retake": "Retake",
  "capture.ocr_retry": "Try again",
  "capture.ocr_page_label": "Photo {n}",
  "capture.ocr_remove_page": "Remove",
  "capture.ocr_add_camera_page": "Add another",
  "capture.ocr_add_library_page": "Add photo",
  "capture.ocr_selector_hint": "Tap the lines you want to save (multi-select)",
  "capture.ocr_no_sentences": "No text detected. Try another photo.",
  "capture.ocr_use_selected": "Use selected",
  "capture.saved": "Saved",
  "capture.missing_content": "Please enter some text",

  "signin.title": "Sign in",
  "signin.subtitle_email": "We'll email you a verification code or magic link",
  "signin.subtitle_code": "Enter the verification code from your email, or tap the link in it.",
  "signin.already_in": "You're already signed in:",
  "signin.email_placeholder": "you@example.com",
  "signin.send_code": "Send code / link",
  "signin.sending": "Sending...",
  "signin.code_sent": "Check your inbox. Tap the link or enter the code below.",
  "signin.code_hint": "Enter the verification code from your email.",
  "signin.code_placeholder": "Verification code",
  "signin.verify": "Sign in with code",
  "signin.verifying": "Verifying...",
  "signin.change_email": "Change email",
  "signin.resend": "Send again",
  "signin.error_rate_limit": "Too many requests. Please try again in about an hour.",
  "signin.error_expired": "Code expired. Please request a new email.",
  "signin.error_empty_code": "Please enter your verification code.",
  "signin.error_invalid_code": "That code doesn't match. Double-check and try again.",
  "signin.error_generic": "Please try again in a moment.",

  "settings.title": "Settings",
  "settings.account": "Account",
  "settings.not_signed_in": "Not signed in (local only)",
  "settings.sign_in": "Sign in",
  "settings.sign_out": "Sign out",
  "settings.delete_account": "Delete account",
  "settings.delete_account_confirm_title": "Delete your account?",
  "settings.delete_account_confirm_body":
    "Synced cloud records, images, and local app data on this device will be removed. This cannot be undone.",
  "settings.delete_account_confirm": "Delete",
  "settings.delete_account_cancel": "Cancel",
  "settings.delete_account_busy": "Deleting...",
  "settings.delete_account_error": "Couldn’t delete the account. Please try again shortly.",
  "settings.legal": "About",
  "settings.privacy": "Privacy Policy",
  "settings.support": "Support",

  "settings.pro": "Quote Pro",
  "settings.pro_desc": "Watermark-free shares · archive notes to Markdown / Obsidian / Notion",
  "settings.pro_desc_active": "Pro is on. Archive export and watermark-free sharing are unlocked.",
  "settings.pro_row_watermark": "Remove watermark on image shares",
  "settings.pro_row_archive": "Export to Markdown · Obsidian · Notion",
  "settings.pro_cta": "Explore Pro",
  "settings.pro_archive_cta": "Export archive",
  "settings.pro_manage": "Manage · Restore",
  "settings.pro_soon": "Pro coming soon",

  "settings.support_dev": "Support the developer",
  "settings.support_dev_desc": "Quote is built by one person. A little support goes a long way.",
  "settings.support_dev_not_pro": "Tips are one-time support and do not unlock Pro features.",
  "settings.support_dev_cta": "Donate",
  "settings.support_dev_soon": "Donations coming soon",

  "pro.title": "Quote Pro",
  "pro.subtitle": "Keep capturing for free — unlock cleaner shares and archives.",
  "pro.badge_active": "Active",
  "pro.feature_watermark": "Share images without the Quote watermark",
  "pro.feature_markdown": "Export reading notes as Markdown (.md)",
  "pro.feature_obsidian": "Open a new note directly in Obsidian",
  "pro.feature_notion": "Archive to a Notion page",
  "pro.subscribe_cta": "Start for {price}/mo",
  "pro.restore": "Restore purchases",
  "pro.restore_ok": "Pro restored.",
  "pro.restore_none": "No subscription found to restore.",
  "pro.purchase_error": "Couldn’t complete the subscription. Check that the store product is ready, or try again shortly.",
  "pro.preview_note": "Preview mode — you can try Pro UI without a real charge.",

  "archive.title": "Export archive",
  "archive.subtitle": "Send your Quote notes to another app. Pro only.",
  "archive.md": "Markdown file",
  "archive.md_desc": "Share or save a .md file",
  "archive.md_ok": "Markdown file is ready.",
  "archive.obsidian": "Obsidian",
  "archive.obsidian_desc": "Open as a new Obsidian note (file share if too long)",
  "archive.obsidian_uri_ok": "Opened Obsidian. If it didn’t launch, try Markdown export instead.",
  "archive.obsidian_file_ok": "Note is long — sharing a .md file. Save it into your vault.",
  "archive.copy": "Copy to clipboard",
  "archive.copy_desc": "Copy Markdown to paste into Notion etc.",
  "archive.copy_ok": "Copied to clipboard.",
  "archive.notion": "Send to Notion",
  "archive.notion_desc": "Create an archive page under your linked parent",
  "archive.notion_show_config": "Notion connection settings",
  "archive.notion_hide_config": "Hide connection settings",
  "archive.notion_help": "You need a Notion Internal Integration token and a parent page ID (invite the integration to that page). The token stays on this device only.",
  "archive.notion_token_ph": "secret_… or ntn_… token",
  "archive.notion_page_ph": "Parent page ID (32 chars)",
  "archive.notion_save": "Save connection",
  "archive.notion_need_config": "Save your Notion token and parent page ID first.",
  "archive.notion_ok": "Created a page in Notion.",
  "archive.notion_ok_url": "Created in Notion: {url}",
  "archive.notion_error": "Notion error: {msg}",
  "archive.empty": "No notes to export yet.",
  "archive.cancelled": "Share cancelled.",
  "archive.error": "Export failed. Please try again shortly.",

  "tip.title": "Buy the developer a coffee",
  "tip.subtitle": "If Quote helps you, feel free to leave a tip.",
  "tip.tier_small": "Small tip",
  "tip.tier_medium": "Medium tip",
  "tip.tier_large": "Large tip",
  "tip.unavailable": "Tips aren’t available right now. Please try again in the app.",
  "tip.error": "Couldn’t load the products.",
  "tip.retry": "Try again",
  "tip.purchase_error": "The purchase didn’t go through. Please try again shortly.",
  "tip.thanks_title": "Thank you so much",
  "tip.thanks_body": "Your support helps keep Quote growing.",

  "settings.store_images": "Keep original photos",
  "settings.store_images_desc": "When off, photos are discarded after OCR",

  "settings.language": "Language",
  "settings.language_desc": "Change the app's display language",

  "settings.sync": "Sync",
  "settings.sync_now": "Sync now",
  "settings.sync_network": "Network",
  "settings.sync_online": "Online",
  "settings.sync_offline": "Offline",
  "settings.sync_login": "Signed in",
  "settings.sync_login_none": "No (local only)",
  "settings.sync_last": "Last sync",
  "settings.sync_pending": "Pending uploads",
  "settings.sync_pending_count": "{count}",
  "settings.sync_failed": "Failed syncs",
  "settings.sync_retry_failed": "Retry",
  "settings.sync_recent": "Recent",

  "settings.covers": "Auto covers",
  "settings.covers_retry": "Retry covers",
  "settings.covers_searching": "Searching...",
  "settings.covers_desc":
    "Missing covers are resolved by title language: Kakao → Naver → Open Library → Google. Kakao/Naver run only when API keys are set in `.env.local`; otherwise Open Library and Google are used. Rebuild the app after changing env. Internet required.",
  "settings.covers_on": "On",
  "settings.covers_off": "Off",
  "settings.covers_providers_status":
    "Book APIs: Kakao {kakao}, Naver {naver}, Open Library {openLibrary}, Google {google}",
  "settings.covers_total": "Total books",
  "settings.covers_already": "Already have covers",
  "settings.covers_tried": "Tried",
  "settings.covers_result": "Succeeded / Failed",
  "settings.covers_book_count": "{count}",
  "settings.covers_no_books": "No books yet. Save a quote first.",
  "settings.covers_match": "matched",
  "settings.covers_fail": "no match",
  "settings.covers_and_more": "...and {count} more",

  "privacy.title": "Privacy Policy",
  "privacy.updated": "Last updated: 2026-07-23",
  "privacy.intro":
    "Quote helps you capture lines from books. Quote is operated by Tealdot (tealdot.dev).",
  "privacy.s1_title": "1. Information we collect",
  "privacy.s1_email": "Email — when you sign in (magic link / verification code)",
  "privacy.s1_content": "Quotes, notes, book titles/authors, and related content you save",
  "privacy.s1_photos":
    "Photos for OCR — cloud upload only if “Keep original photos” is on",
  "privacy.s1_local": "On-device storage for offline use",
  "privacy.s2_title": "2. How we use it",
  "privacy.s2_auth": "Authentication and sync across devices",
  "privacy.s2_quotes": "Saving and organizing quotes",
  "privacy.s2_covers": "Automatic book-cover matching via third-party book APIs",
  "privacy.s3_title": "3. Third parties",
  "privacy.s3_supabase": "Supabase — auth, database, optional image storage",
  "privacy.s3_books": "Google Books and (if configured) Kakao/Naver book APIs",
  "privacy.s3_no_ads": "We don’t use advertising or tracking SDKs, and we don’t sell personal data.",
  "privacy.s4_title": "4. Retention & deletion",
  "privacy.s4_body":
    "Delete account in Settings removes your account, synced data, related cloud images, and local app data on this device.",
  "privacy.s5_title": "5. Contact",
  "privacy.s5_contact": "Contact via the website",

  "book.unknown_title": "Untitled",
  "book.back_label": "Back",
  "book.cover_alt": "{title} cover",
  "book.back_to_library": "Library",
  "book.quote_count": "{count} quotes",
  "book.empty_quotes": "No quotes yet for this book",
  "book.add_quote": "Add a quote to this book",
  "book.add_quote_hint": "Enter a passage to save under this book.",

  "quote.action_title": "Quote actions",
  "quote.edit": "Edit",
  "quote.delete": "Delete",
  "quote.deleting": "Deleting...",
  "quote.delete_confirm_title": "Delete this quote? This can't be undone.",
  "quote.cancel": "Cancel",
  "quote.action_default_title": "Choose an action",
  "quote.deleted": "Deleted",
  "quote.expand": "Read more",
  "quote.collapse": "Show less",
  "quote.favorite": "Favorite",
  "quote.share_image": "Share as image",

  "edit.title": "Edit quote",
  "edit.save": "Save",
  "edit.saving": "Saving...",
  "edit.cancel": "Cancel",
  "edit.book_title_placeholder": "Book title (leave empty to unlink)",

  "share.title": "Share as image",
  "share.size_square": "Square (1080×1350)",
  "share.size_story": "Story (1080×1920)",
  "share.save_photo": "Save to Photos",
  "share.share": "Share",
  "share.generating": "Generating image...",
  "share.promoted_notice":
    "This quote is long — switched to the 9:16 story format automatically.",
  "share.too_long_warning":
    "This quote is very long and part of it may be clipped. Multi-page sharing is coming soon.",
  "share.select_sentences_hint": "Pick which sentences to share",
  "share.sentences_none_selected": "No sentences selected",
  "share.watermark_off_pro": "Share without watermark",
  "share.watermark_upsell": "Remove the watermark with Quote Pro",
};

const ja: Dict = {
  "nav.home": "ホーム",
  "nav.library": "本棚",
  "nav.capture": "記録",
  "nav.settings": "設定",

  "common.save": "保存",
  "common.cancel": "キャンセル",
  "common.delete": "削除",
  "common.edit": "編集",
  "common.share": "共有",
  "common.retry": "再試行",
  "common.close": "閉じる",
  "common.confirm": "確認",
  "common.back": "戻る",
  "common.loading": "読み込み中...",
  "common.ok": "OK",

  "time.never": "まだありません",
  "time.just_now": "たった今",
  "time.seconds_ago": "{n}秒前",
  "time.minutes_ago": "{n}分前",
  "time.hours_ago": "{n}時間前",
  "time.days_ago": "{n}日前",

  "home.brand": "Quote",
  "home.daily_title": "今日の一節",
  "home.shuffle": "別の一節を見る",
  "home.recent_title": "最近の記録",
  "home.empty": "まだ記録がありません",
  "home.empty_cta": "最初の一節を記録してみましょう",
  "home.greeting_morning": "おはようございます",
  "home.greeting_afternoon": "こんにちは",
  "home.greeting_evening": "こんばんは",
  "home.welcome_content": "最初の一節を記録してみましょう。カメラで撮れば自動で文字が認識されます。",
  "home.welcome_book": "ようこそ",

  "library.title": "本棚",
  "library.count": "{books}冊 · {quotes}件",
  "library.empty": "まだ本がありません",
  "library.empty_initial": "最初の一節を記録すると、ここに本が並びます",
  "library.empty_search": "該当する結果がありません",
  "library.search_placeholder": "タイトルや著者で検索...",
  "library.quote_count": "{count}件",
  "library.unassigned": "未分類の一節",
  "library.unassigned_short": "未分類",
  "library.unassigned_desc": "本の情報なしで記録した一節",
  "library.unassigned_count": "{count}件",
  "library.last_quote_at": "最終 {time}",

  "capture.title": "記録する",
  "capture.subtitle": "一行のインスピレーションを残しましょう",
  "capture.from_camera": "写真",
  "capture.from_photo": "写真",
  "capture.from_voice": "音声",
  "capture.type_directly": "直接入力",
  "capture.quote_placeholder": "心に残った一節を入力してください。",
  "capture.book_info_label": "本の情報",
  "capture.book_title_placeholder": "本のタイトル",
  "capture.book_author_placeholder": "著者",
  "capture.recent_books": "最近読んだ本",
  "capture.thought_label": "あなたの感想",
  "capture.thought_placeholder": "この一節について自由に書いてみましょう。",
  "capture.save": "保存する",
  "capture.save_error": "保存に失敗しました。しばらくしてから再試行してください。",
  "capture.saving": "保存中...",
  "capture.ocr_title": "写真から記録",
  "capture.ocr_from_camera": "カメラで撮影",
  "capture.ocr_from_library": "写真から選択",
  "capture.ocr_recognizing": "文字を認識中…",
  "capture.ocr_failed": "認識に失敗",
  "capture.ocr_retake": "撮り直す",
  "capture.ocr_retry": "再試行",
  "capture.ocr_page_label": "{n}枚目の写真",
  "capture.ocr_remove_page": "削除",
  "capture.ocr_add_camera_page": "もう1枚撮影",
  "capture.ocr_add_library_page": "写真を追加",
  "capture.ocr_selector_hint": "記録する一節をタップ (複数選択可)",
  "capture.ocr_no_sentences": "認識された一節がありません。もう一度撮影してください。",
  "capture.ocr_use_selected": "選択した一節を使う",
  "capture.saved": "保存しました",
  "capture.missing_content": "内容を入力してください",

  "signin.title": "ログイン",
  "signin.subtitle_email": "メールで認証コードまたはマジックリンクをお送りします",
  "signin.subtitle_code": "メールに届いた認証コードを入力するか、メール内のリンクを押してください。",
  "signin.already_in": "すでにログインしています:",
  "signin.email_placeholder": "you@example.com",
  "signin.send_code": "コード / リンクを送る",
  "signin.sending": "送信中...",
  "signin.code_sent": "受信箱をご確認ください。リンクをタップしても、コードを入力してもOKです。",
  "signin.code_hint": "メールに届いた認証コードを入力してください。",
  "signin.code_placeholder": "認証コード",
  "signin.verify": "コードでログイン",
  "signin.verifying": "確認中...",
  "signin.change_email": "メールを変更",
  "signin.resend": "コードを再送",
  "signin.error_rate_limit": "メール送信が多すぎます。しばらく経ってから(約1時間)再試行してください。",
  "signin.error_expired": "コードの有効期限が切れました。メールを再送してください。",
  "signin.error_empty_code": "認証コードを入力してください。",
  "signin.error_invalid_code": "コードが正しくありません。ご確認ください。",
  "signin.error_generic": "しばらく経ってから再試行してください。",

  "settings.title": "設定",
  "settings.account": "アカウント",
  "settings.not_signed_in": "未ログイン (ローカル保存のみ)",
  "settings.sign_in": "ログイン",
  "settings.sign_out": "ログアウト",
  "settings.delete_account": "アカウント削除",
  "settings.delete_account_confirm_title": "アカウントを削除しますか？",
  "settings.delete_account_confirm_body":
    "クラウドに同期された記録・画像と、この端末のアプリデータが削除されます。元に戻せません。",
  "settings.delete_account_confirm": "削除する",
  "settings.delete_account_cancel": "キャンセル",
  "settings.delete_account_busy": "削除中...",
  "settings.delete_account_error": "アカウント削除に失敗しました。しばらくしてから再試行してください。",
  "settings.legal": "情報",
  "settings.privacy": "プライバシーポリシー",
  "settings.support": "サポート",

  "settings.pro": "Quote Pro",
  "settings.pro_desc": "透かしなし共有 · Markdown / Obsidian / Notion へアーカイブ",
  "settings.pro_desc_active": "Pro が有効です。アーカイブと透かしなし共有が使えます。",
  "settings.pro_row_watermark": "画像共有時の透かしを削除",
  "settings.pro_row_archive": "Markdown · Obsidian · Notion へ書き出し",
  "settings.pro_cta": "Pro を見る",
  "settings.pro_archive_cta": "アーカイブを書き出す",
  "settings.pro_manage": "サブスク管理 · 復元",
  "settings.pro_soon": "Pro は準備中です",

  "settings.support_dev": "開発者を応援する",
  "settings.support_dev_desc": "Quoteは個人で開発しているアプリです。応援が大きな力になります。",
  "settings.support_dev_not_pro": "支援は一度きりの応援で、Pro 機能の解除にはなりません。",
  "settings.support_dev_cta": "支援する",
  "settings.support_dev_soon": "支援機能は準備中です",

  "pro.title": "Quote Pro",
  "pro.subtitle": "記録はそのまま。共有とアーカイブだけもう一段。",
  "pro.badge_active": "利用中",
  "pro.feature_watermark": "画像共有時の透かしを削除",
  "pro.feature_markdown": "読書ノートを Markdown (.md) で書き出し",
  "pro.feature_obsidian": "Obsidian で新しいノートを開く",
  "pro.feature_notion": "Notion ページへアーカイブ",
  "pro.subscribe_cta": "月額 {price} で始める",
  "pro.restore": "購入を復元",
  "pro.restore_ok": "Pro を復元しました。",
  "pro.restore_none": "復元できるサブスクが見つかりませんでした。",
  "pro.purchase_error": "サブスクを完了できませんでした。ストア商品の準備状況を確認するか、しばらくして再試行してください。",
  "pro.preview_note": "プレビューモード — 実際の課金なしで Pro UI を確認できます。",

  "archive.title": "アーカイブ書き出し",
  "archive.subtitle": "Quote の読書ノートを外部アプリへ送ります。Pro 専用。",
  "archive.md": "Markdown ファイル",
  "archive.md_desc": ".md を共有または保存",
  "archive.md_ok": "Markdown ファイルを準備しました。",
  "archive.obsidian": "Obsidian",
  "archive.obsidian_desc": "Obsidian で新規ノートを開く（長い場合はファイル共有）",
  "archive.obsidian_uri_ok": "Obsidian を開きました。起動しない場合は Markdown 書き出しを試してください。",
  "archive.obsidian_file_ok": "ノートが長いため .md を共有します。vault フォルダに保存してください。",
  "archive.copy": "クリップボードにコピー",
  "archive.copy_desc": "Markdown をコピーして Notion などに貼り付け",
  "archive.copy_ok": "クリップボードにコピーしました。",
  "archive.notion": "Notion に送る",
  "archive.notion_desc": "連携した親ページの下にアーカイブページを作成",
  "archive.notion_show_config": "Notion 連携設定",
  "archive.notion_hide_config": "連携設定を閉じる",
  "archive.notion_help": "Notion Internal Integration のトークンと、Integration を招待した親ページ ID が必要です。トークンはこの端末にのみ保存されます。",
  "archive.notion_token_ph": "secret_… または ntn_… トークン",
  "archive.notion_page_ph": "親ページ ID（32文字）",
  "archive.notion_save": "連携を保存",
  "archive.notion_need_config": "先に Notion トークンと親ページ ID を保存してください。",
  "archive.notion_ok": "Notion にページを作成しました。",
  "archive.notion_ok_url": "Notion に作成しました: {url}",
  "archive.notion_error": "Notion エラー: {msg}",
  "archive.empty": "書き出すノートがまだありません。",
  "archive.cancelled": "共有をキャンセルしました。",
  "archive.error": "書き出しに失敗しました。しばらくして再試行してください。",

  "tip.title": "開発者にコーヒーを一杯",
  "tip.subtitle": "アプリが役に立ったら、気軽に応援してください。",
  "tip.tier_small": "小さな応援",
  "tip.tier_medium": "しっかり応援",
  "tip.tier_large": "大きな応援",
  "tip.unavailable": "現在、支援を受け付けていません。アプリで再度お試しください。",
  "tip.error": "商品情報を読み込めませんでした。",
  "tip.retry": "再試行",
  "tip.purchase_error": "購入を完了できませんでした。しばらくして再試行してください。",
  "tip.thanks_title": "本当にありがとうございます",
  "tip.thanks_body": "いただいた応援はQuoteの改善に大きく役立ちます。",

  "settings.store_images": "元画像を保存",
  "settings.store_images_desc": "オフにすると認識後の元画像は保存されません",

  "settings.language": "言語",
  "settings.language_desc": "アプリの表示言語を変更します",

  "settings.sync": "同期",
  "settings.sync_now": "今すぐ同期",
  "settings.sync_network": "ネットワーク",
  "settings.sync_online": "オンライン",
  "settings.sync_offline": "オフライン",
  "settings.sync_login": "ログイン",
  "settings.sync_login_none": "なし (ローカルのみ)",
  "settings.sync_last": "最終同期",
  "settings.sync_pending": "アップロード待ち",
  "settings.sync_pending_count": "{count}件",
  "settings.sync_failed": "同期失敗",
  "settings.sync_retry_failed": "再試行",
  "settings.sync_recent": "最近の結果",

  "settings.covers": "自動カバー",
  "settings.covers_retry": "カバーを再取得",
  "settings.covers_searching": "検索中...",
  "settings.covers_desc":
    "カバーがない本はタイトル言語に応じて Kakao → Naver → Open Library → Google の順で探します。Kakao/Naver は `.env.local` にキーがあるときだけ先に使われ、なければ Open Library・Google を使います。キー変更後はアプリを再ビルドしてください。インターネットが必要です。",
  "settings.covers_on": "利用",
  "settings.covers_off": "未設定",
  "settings.covers_providers_status":
    "書籍API: Kakao {kakao}, Naver {naver}, Open Library {openLibrary}, Google {google}",
  "settings.covers_total": "本の合計",
  "settings.covers_already": "カバーあり",
  "settings.covers_tried": "試行",
  "settings.covers_result": "成功 / 失敗",
  "settings.covers_book_count": "{count}冊",
  "settings.covers_no_books": "本棚にまだ本がありません。まず一節を保存してください。",
  "settings.covers_match": "一致",
  "settings.covers_fail": "未一致",
  "settings.covers_and_more": "...他 {count}件",

  "privacy.title": "プライバシーポリシー",
  "privacy.updated": "最終更新: 2026-07-23",
  "privacy.intro":
    "Quote は本の一文を記録・保管するアプリです。Quote は Tealdot (tealdot.dev) が運営しています。",
  "privacy.s1_title": "1. 収集する情報",
  "privacy.s1_email": "メール — ログイン時（マジックリンク / 認証コード）",
  "privacy.s1_content": "引用、メモ、書名・著者など保存したコンテンツ",
  "privacy.s1_photos":
    "写真 — OCR用。「元画像を保存」がオンの場合のみクラウドにアップロードされることがあります",
  "privacy.s1_local": "端末内のローカル保存（オフライン用）",
  "privacy.s2_title": "2. 利用目的",
  "privacy.s2_auth": "認証と端末間同期",
  "privacy.s2_quotes": "引用の保存・本棚管理",
  "privacy.s2_covers": "表紙の自動マッチ（外部の書籍 API）",
  "privacy.s3_title": "3. 第三者",
  "privacy.s3_supabase": "Supabase — 認証、DB、（任意）画像保存",
  "privacy.s3_books": "Google Books および（設定時）Kakao/Naver 書籍 API",
  "privacy.s3_no_ads": "広告・追跡 SDK は使わず、個人情報を販売しません。",
  "privacy.s4_title": "4. 保管と削除",
  "privacy.s4_body":
    "設定のアカウント削除で、アカウント・同期データ・関連クラウド画像とこの端末のアプリデータが削除されます。",
  "privacy.s5_title": "5. お問い合わせ",
  "privacy.s5_contact": "ウェブサイトから問い合わせる",

  "book.unknown_title": "タイトルなし",
  "book.back_label": "戻る",
  "book.cover_alt": "{title} の表紙",
  "book.back_to_library": "本棚",
  "book.quote_count": "{count}件の一節",
  "book.empty_quotes": "この本の一節はまだありません",
  "book.add_quote": "この本に一節を追加",
  "book.add_quote_hint": "この本に保存する一節を入力してください。",

  "quote.action_title": "この一節の操作",
  "quote.edit": "編集",
  "quote.delete": "削除",
  "quote.deleting": "削除中...",
  "quote.delete_confirm_title": "この一節を削除しますか?元に戻せません。",
  "quote.cancel": "キャンセル",
  "quote.action_default_title": "操作を選択",
  "quote.deleted": "削除しました",
  "quote.expand": "続きを読む",
  "quote.collapse": "折りたたむ",
  "quote.favorite": "お気に入り",
  "quote.share_image": "画像として共有",

  "edit.title": "一節を編集",
  "edit.save": "保存",
  "edit.saving": "保存中...",
  "edit.cancel": "キャンセル",
  "edit.book_title_placeholder": "本のタイトル (空欄でリンク解除)",

  "share.title": "画像として共有",
  "share.size_square": "正方形 (1080×1350)",
  "share.size_story": "ストーリー (1080×1920)",
  "share.save_photo": "写真に保存",
  "share.share": "共有",
  "share.generating": "画像生成中...",
  "share.promoted_notice":
    "文章が長いため、縦型 (9:16) サイズに自動切り替えしました。",
  "share.too_long_warning":
    "文章が非常に長いため、一部が切れる可能性があります。分割保存は近日対応予定です。",
  "share.select_sentences_hint": "共有する文を選んでください",
  "share.sentences_none_selected": "文が選択されていません",
  "share.watermark_off_pro": "透かしなしで共有",
  "share.watermark_upsell": "透かしなし共有は Quote Pro",
};

export const translations: Record<Language, Dict> = { ko, en, ja };
